...
start(Port) ->
	spawn(fun() -> run(Port) end).

stop(Pid) ->
	Pid ! stop.

run(Port) ->
	Ret = gen_tcp:listen(Port,
...
accept(Listen) ->
	{ok, ClientSocket} = gen_tcp:accept(Listen),
	%spawn(fun() -> ?MODULE:accept(Listen) end),
	%gen_tcp:close(Listen),
	spawn(fun() -> supervisor(ClientSocket) end),
	?MODULE:accept(Listen).

supervisor(ClientSocket) ->	
	{ok, ServerSocket} = gen_tcp:connect({127,0,0,1}, 25565,
		[binary, {reuseaddr, true}, {active, false}, {packet, raw}]),
	process_flag(trap_exit, true),
	Pid1 = spawn_link(fun() -> loop("C->S", ClientSocket, ServerSocket) end),
	Pid2 = spawn_link(fun() -> loop("S->C", ServerSocket, ClientSocket) end),
	gen_tcp:controlling_process(ClientSocket, Pid1),
	gen_tcp:controlling_process(ServerSocket, Pid2),
	io:format("new connection, C->S ~p and S->C ~p~n", [Pid1, Pid2]),
	supervisor(Pid1, Pid2, ClientSocket, ServerSocket).
...
supervisor(Pid1, Pid2, ClientSocket, ServerSocket) ->
	receive
		{tcp_closed, ClientSocket} ->
			...
			Pid1 ! stop,
			Pid2 ! stop,
			io:format("client connection closed, stopping");
		{tcp_closed, ServerSocket} ->
			gen_tcp:close(ClientSocket),
			Pid1 ! stop,
			Pid2 ! stop,
			io:format("server connection closed, stopping");
		{'EXIT', _Pid, _Reason} ->
			gen_tcp:close(ServerSocket),
			gen_tcp:close(ClientSocket),
			Pid1 ! stop,
			Pid2 ! stop,
			io:format("[supervisor] stopping~n");
		X ->
			io:format("[supervisor] unknown message ~p~n", [X]),
			?MODULE:supervisor(Pid1, Pid2, ClientSocket, ServerSocket)
	end.

loop(Name, Socket1, Socket2) ->
	receive
		stop ->
		...
		X ->
			...
			?MODULE:loop(Name, Socket1, Socket2)
		after 0 ->
			case decode_packet(Socket1) of
				{ok, Packet} ->
					io:format("[~s] packet: ~p~n", [Name, Packet]),
					Bin = encode_packet(Packet),
					gen_tcp:send(Socket2, Bin),
					?MODULE:loop(Name, Socket1, Socket2);
				{error, Reason} ->
					io:format("[~s] stopping with ~p~n", [Name, {error, Reason}]),
					{error, Reason}
			end
	end.
...
decode_packet(Socket) ->
	...
	case Recv of
		{ok, <<Id>>} ->
			{Id, Name, TypeParamList} = packets:get_by_id(Id),
...
encode_packet({Name, ParamList}) ->
	{Id, Name, TypeParamList} = packets:get_by_name(Name),
